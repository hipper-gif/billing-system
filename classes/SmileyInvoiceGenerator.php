 ...
 * 領収書PDF生成クラス
 */
class SmileyReceiptPDF extends SmileyInvoicePDF {
    
    /**
     * 領収書PDF生成
     */
    public function generateReceiptPDF($receiptData) {
        $this->initializePDF();
        $this->addReceiptPage($receiptData);
        
        $filename = $this->generateReceiptFilename($receiptData);
        $filepath = $this->savePDF($filename);
     ...
     * 領収書ページ追加
     */
    private function addReceiptPage($receipt) {
        $this->addReceiptHeader($receipt);
        $this->addReceiptCompanyInfo();
        $this->addReceiptBillingInfo($receipt);
        $this->addReceiptAmount($receipt);
        $this->addReceiptFooter($receipt);
    }
    
    /**
     * 領収書ヘッダー
     */
    private function addReceiptHeader($receipt) {
        // Smiley Kitchenロゴ配置
        if (file_exists($this->logoPath)) {
            $this->pdf->Image($this->logoPath, 20, 15, 40, 0, 'PNG');
        }
        ...
        $this->pdf->SetTextColor(...self::BRAND_GREEN);
        $this->pdf->SetXY(70, 20);
        $this->pdf->Cell(0, 10, '領収書', 0, 1, 'L');
        
        // 領収書番号
        $this->pdf->SetFont('DejaVu', 'B', 14);
        $this->pdf->SetTextColor(...self::BRAND_GRAY);
        $this->pdf->SetXY(70, 35);
        $this->pdf->Cell(0, 8, 'Receipt No: ' . $receipt['receipt_number'], 0, 1, 'L');
        
        // 発行日
        $this->pdf->SetFont('DejaVu', '', 11);
        $this->pdf->SetXY(70, 45);
        $this->pdf->Cell(0, 6, '発行日: ' . $this->formatDate($receipt['issue_date']), 0, 1, 'L');
     ...
     * 金額セクション（領収書用）
     */
    private function addReceiptAmount($receipt) {
        $y = 120;
        ...
        $this->pdf->SetTextColor(...self::BRAND_GRAY);
        $this->pdf->SetXY(20, $y);
        $this->pdf->Cell(0, 8, $receipt['billing_company_name'] . ' 様', 0, 1, 'L');
        
        // 金額（大きく表示）
        $this->pdf->SetFont('DejaVu', 'B', 20);
        $this->pdf->SetTextColor(...self::BRAND_GREEN);
        $this->pdf->SetXY(20, $y + 20);
        $this->pdf->Cell(0, 12, '金額　￥' . number_format($receipt['total_amount']) . '－', 0, 1, 'C');
        
        // 但し書き
        $this->pdf->SetFont('DejaVu', '', 12);
        $this->pdf->SetTextColor(...self::BRAND_GRAY);
        $this->pdf->SetXY(20, $y + 40);
        $description = $receipt['description'] ?? '配食サービス料として';
        $this->pdf->Cell(0, 8, '但し　' . $description, 0, 1, 'L');
        
        // 収入印紙（5万円以上の場合）
        if ($receipt['total_amount'] >= 50000) {
            $this->pdf->SetXY(150, $y + 60);
            $this->pdf->Cell(40, 20, '収入印紙', 1, 0, 'C');
        }
    }
     ...
     * 領収書ファイル名生成
     */
    private function generateReceiptFilename($receipt) {
        $date = date('Ymd');
        $receiptNumber = preg_replace('/[^a-zA-Z0-9-_]/', '_', $receipt['receipt_number']);
     ...
     * 領収書フッター
     */
    private function addReceiptFooter($receipt) {
        $y = 220;
